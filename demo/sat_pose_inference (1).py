...
def prepare_output_dirs(args, cfg, prefix='output/'):
    folder = 'sat_pose_inferences'
    pose_dir = os.path.join(prefix, folder, cfg.DATASET.DATASET, args.data_name, 'poses')
    box_dir = os.path.join(prefix, folder,cfg.DATASET.DATASET, args.data_name, 'boxes')
    hm_dir = os.path.join(prefix, folder,cfg.DATASET.DATASET, args.data_name,'heatmaps')
...
def main(image, bbox, args):
    ...
    csv_output_filename = os.path.abspath(os.path.join('output', 'sat_pose_inferences', cfg.DATASET.DATASET, args.data_name,'pose-data.csv'))
    csv_output_rows = []
    ...
    frames = [image]
    idx_map = {list(bbox)[0]: 0}
    print(idx_map)

    bboxes_all_imgs = bbox
    ...
    for idx,frame in enumerate(frames):
        ...
        if args.writeBoxFrames:
            ...
            print(os.path.join (box_dir,f"{img_name}_box_{idx}.jpg"))
        if not boxes:
            continue
        ...
        pose_preds, maxvals, output = get_pose_estimation_prediction(pose_model, image_pose, center, scale)

        new_csv_row = [idx]
        for idx1, mat in enumerate(pose_preds[0]):
            x_coord, y_coord = int(mat[0]), int(mat[1])
            cv2.circle(image_pose, (x_coord, y_coord), 100, (0, 255, 255), 5)
            new_csv_row.extend([x_coord, y_coord])
            print(f"{idx1}, {[x_coord,y_coord]}")

        csv_output_rows.append(new_csv_row)
        save_dir = pose_dir+f'/{img_name}_pose.jpg'
        cv2.imwrite(save_dir, image_pose)
        print(save_dir)
    # write csv
    csv_headers = ['frame']
